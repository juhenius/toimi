...
RUN dotnet restore src/toimi.tools.muistutin/toimi.tools.muistutin.csproj

COPY src/ src/
RUN dotnet publish src/toimi.tools.muistutin/toimi.tools.muistutin.csproj -c Release -o /app
...
EXPOSE 8080
ENTRYPOINT ["dotnet", "toimi.tools.muistutin.dll"]
