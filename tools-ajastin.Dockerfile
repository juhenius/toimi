...
RUN dotnet restore src/toimi.tools.ajastin/toimi.tools.ajastin.csproj

COPY src/ src/
RUN dotnet publish src/toimi.tools.ajastin/toimi.tools.ajastin.csproj -c Release -o /app
...
EXPOSE 8080
ENTRYPOINT ["dotnet", "toimi.tools.ajastin.dll"]
