...
RUN dotnet restore src/toimi.tools.muistio/toimi.tools.muistio.csproj

COPY src/ src/
RUN dotnet publish src/toimi.tools.muistio/toimi.tools.muistio.csproj -c Release -o /app
...
EXPOSE 8080
ENTRYPOINT ["dotnet", "toimi.tools.muistio.dll"]
