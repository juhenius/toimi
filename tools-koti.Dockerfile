...
RUN dotnet restore src/toimi.tools.koti/toimi.tools.koti.csproj

COPY src/ src/
RUN dotnet publish src/toimi.tools.koti/toimi.tools.koti.csproj -c Release -o /app
...
EXPOSE 8080
ENTRYPOINT ["dotnet", "toimi.tools.koti.dll"]
